...
void Task_Snail(void *Parameters) {
  ...
  float dutyCycleRightSnailProgress1 =
      0; //存储需要的两个过程（初始到启动，启动到你想要的速度）
      ...
      if (dutyCycleRightSnailProgress1 <= 1) { //初始状态
        ...
        if (dutyCycleLeftSnailProgress1 <= 1) {
          ...
          if (snailLeftState == 0) {
            ...
            if (dutyCycleLeftSnailProgress2 <= 1) {
              dutyCycleLeftSnailTarget = RAMP(dutyCycleMiddle, dutyCycleEnd,
                                              dutyCycleLeftSnailProgress2);
        ...
        if (snailRightState == 0) { //初始状态停留100ms
          ...
          if (dutyCycleRightSnailProgress2 <= 1) { //启动状态
            dutyCycleRightSnailTarget =
                RAMP(dutyCycleMiddle, dutyCycleEnd,
...
void Task_Gimbal(void *Parameters) {
  ...
  while (1) {
    ...
    if (ABS(remoteData.rx) > 30)
      yawAngleTarget += -remoteData.rx / 660.0f * 20 * 0.01;
    if (ABS(remoteData.ry) > 30)
      pitchAngleTarget += -remoteData.ry / 660.0f * 30 * 0.01;
    ...
    //开机时pitch轴匀速抬起
    pitchAngleTargetRamp =
        RAMP(pitchRampStart, pitchAngleTarget, pitchRampProgress);
    ...
    Can_Send(CAN1, 0x1FF, -PID_Cloud_YawSpeed.output,
             -PID_Cloud_RollSpeed.output, -PID_Cloud_PitchSpeed.output, 0);
    ...
    DebugData.debug5 = rollAngle * 1000;
...
void Task_Fire(void *Parameters) {
  ...
  PID_Init(&PID_StirSpeed, 3, 0.01, 0, 10000, 1000); // 1.8
