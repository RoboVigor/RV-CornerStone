...
void Task_Chassis(void *Parameters) {
    ...
    // 初始化角速度PID
    PID_Init(&PID_Chassis_Left, 6, 0, 0, 4000, 2000);
    PID_Init(&PID_Chassis_Right, 6, 0, 0, 4000, 2000);

    PID_Init(&PID_Stabilizer_Yaw_Angle, 0, 0, 0, 4000, 2000);
    PID_Init(&PID_Stabilizer_Yaw_Speed, 0, 0, 0, 4000, 2000);
    ...
    while (1) {
        ...
        PID_Calculate(&PID_Stabilizer_Yaw_Angle, 0, Motor_Stabilizer_Yaw.angle);
        PID_Calculate(&PID_Stabilizer_Yaw_Speed, PID_Stabilizer_Yaw_Angle.output, Motor_Stabilizer_Yaw.speed * rpm2rps);

        PID_Calculate(&PID_Stabilizer_Pitch_Angle, 0, Motor_Stabilizer_Pitch.angle);
        ...
        debug1 = PID_Chassis_Left.output;
        debug2 = remoteData.lx;
    }
